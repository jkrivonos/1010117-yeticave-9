...
SELECT lot.description, lot.start_price, lot.creation_date, lot.img_link, lot.final_price, category.name FROM lot INNER JOIN category ON lot.category_id = category.id WHERE lot.winner_user_id IS NULL ORDER BY lot.creation_date DESC;
